...
def cross_entropy_loss_RCF(prediction, labelf, beta):
    ...
    alpha = 1.1  # 超参数，用于增大正样本权重
    mask[label == 1] = alpha * num_negative / (num_positive + num_negative)
    ...
    loss_func = nn.BCEWithLogitsLoss(weight=mask, reduction='sum')
    cost = loss_func(prediction, labelf)
